...
server.get('/category',(req,res)=>{
    //SQL查询语句
    let sql = 'SELECT place,name,price FROM cake1_item';
    //执行SQL查询语句
...
server.get('/show',(req,res)=>{
    let sql='SELECT * FROM cake1_show WHERE img_place1 LIKE ?';
...
server.get('/view',(req,res)=>{
    ...
    name='%'+name+"%"
    //以id为条件进行文章相关信息的查找操作(一会儿还要进行调整)
    let sql = 'SELECT fid,place,name,price,like1,buyCount,IntPrice FROM cake1_item WHERE name LIKE ?';
...
server.post('/register',(req,res)=>{
    ...
    //以用户名为条件进行查找操作,如果用户存在,则产生合法的错误信息
    //如果用户不存在,则将用户的相关信息写入数据表 -- xzqa_author  
    let sql = 'SELECT COUNT(pid) AS count FROM cake1_user WHERE username=?';
  pool.query(sql,[username],(err,results)=>{
        if(err) throw err;  
        //此时代表用户已经存在     
        if(results[0].count){
            ...
            //完成数据写入操作
            let sql = 'INSERT INTO cake1_user(username,passworld,phone) VALUES(?,?,?)';
           pool.query(sql,[username,password,phone],(err,results)=>{
                if(err) throw err;
                res.send({message:'注册成功',code:1});
            });
        }
...
/*server.post('/login',(req,res)=>{
    ...
    let sql = 'SELECT id,username,nickname FROM cake1_user WHERE username=? AND password=MD5(?)';
    pool.query(sql,[username,password],(err,results)=>{
        if(err) throw err;
        if(results.length == 1){            
...
server.post('/login',(req,res)=>{
    ...
    //以用户名和密码为条件进行查找，如果找到，则代表用户登录成功;否则代表登录失败
    let sql = 'SELECT pid,username FROM cake1_user WHERE username=? AND passworld=?';
   pool.query(sql,[username,password],(err,results)=>{
        if(err) throw err;
        if(results.length == 1){            
            res.send({message:'登录成功',code:1,results:results[0]});
